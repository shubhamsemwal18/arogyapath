...
function Map_Disease_with_special_instructions() {
  ...
  let state = useSelector((state) => state.DiseaseMapSpecialInstructionReducer);
  const CommonState = useSelector((state) => state.commonReducer);

  useEffect(() => {
    setIsLoading(true)
    dispatch(ACTIONS.getDiseaseSpecialInstruction())
    dispatch(CommonAction.getDataNotMappedWithDiseaseByParam('special_instruction'))
    
  }, [state.createDiseaseSpecialInstructionSuccess,state.updateDiseaseSpecialInstructionSuccess])

  useEffect(()=>{
    dispatch(CommonAction.getSpecialInstruction())
  },[])


  useEffect(() => {
    if (CommonState.getSpecialInstructionSuccess) {
      setIsLoading(false)
      setAllInstructions(CommonState.getSpecialInstructionSuccess.allInstructions)
      addToast("Success!", {
                appearance: "success",
                content: `Get All instruction Success.`,
              });
    }
    if (CommonState.getSpecialInstructionFailure) {
      setIsLoading(false)
            addToast("Error!", {
                appearance: "error",
                content: `Unable to get instructions.`,
              });
    }

    dispatch(CommonAction.resetToInitialState())

  }, [CommonState.getSpecialInstructionSuccess, CommonState.getSpecialInstructionFailure])
  ...
  useEffect(() => {
    if (state.getDiseaseSpecialInstructionSuccess) {
      setIsLoading(false)
      setAllMappedData(state.getDiseaseSpecialInstructionSuccess.data)
      addToast("Success!", {
                appearance: "success",
                content: `Get All data Success.`,
              });
    }
    if (state.getDiseaseSpecialInstructionFailure) {
      setIsLoading(false)
            addToast("Error!", {
                appearance: "error",
  ...
  }, [state.getDiseaseSpecialInstructionSuccess, state.getDiseaseSpecialInstructionFailure])
  ...
  let handleSubmit = (e) => {
    ...
    if(allDiseasesChecked.length > 0 && allInstructionsChecked.length > 0){
      setIsSubmiting(true)
      dispatch(ACTIONS.createDiseaseSpecialInstruction({
  ...
  useEffect(() => {
    ...
    if (state.createDiseaseSpecialInstructionSuccess) {
    ...
    if (state.createDiseaseSpecialInstructionFailure) {
      setIsSubmiting(false)
      addToast("Error!", {
        appearance: "error",
  ...
  }, [state.createDiseaseSpecialInstructionSuccess, state.createDiseaseSpecialInstructionFailure])
  ...
  let handleUpdate = (e) =>{
    ...
    if(allInstructionsChecked.length > 0  && requestPayload.id && requestPayload.disease_id){
      setIsSubmiting(true)
      dispatch(ACTIONS.updateDiseaseSpecialInstruction({
  ...
  useEffect(() => {
    if (state.updateDiseaseSpecialInstructionSuccess) {
      setRequestPayload({
      })
        addToast("Success!", {
            appearance: "success",
    ...
    if (state.updateDiseaseSpecialInstructionFailure) {
      setIsSubmiting(false)
        addToast("Error!", {
            appearance: "error",
...
}, [state.updateDiseaseSpecialInstructionSuccess, state.updateDiseaseSpecialInstructionFailure])
                  ...
                  <div className="row">
                    <div className="col-md-8">
                      <h1 className={FormStyle.formTitle}>
                        ...
                        <CommonText en="Disease Map Special Instruction" hi="रोग आधारित विशेष निर्देश" />
...
export default Map_Disease_with_special_instructions