...
function Map_Disease_with_diets_not_to_take() {
  ...
  let state = useSelector((state) => state.DiseaseMapDietNotToTakeReducer);
  const CommonState = useSelector((state) => state.commonReducer);

  useEffect(() => {
    setIsLoading(true)
    dispatch(ACTIONS.getDiseaseDietNotToTake())
    dispatch(CommonAction.getDataNotMappedWithDiseaseByParam('diets_not_take'))
    
  }, [state.createDiseaseDietNotToTakeSuccess,state.updateDiseaseDietNotToTakeSuccess])

  useEffect(()=>{
    dispatch(CommonAction.getDietsNotToTake())
  },[])

  useEffect(() => {
    if (CommonState.getDietsNotToTakeSuccess) {
      setAllDiets(CommonState.getDietsNotToTakeSuccess.allDiets)
      addToast("Success!", {
                appearance: "success",
                content: `Get All Diets not to take Success.`,
              });
    }
    if (CommonState.getDietsNotToTakeFailure) {
            addToast("Error!", {
                appearance: "error",
                content: `Unable to get all diets not to take.`,
              });
    }

    dispatch(CommonAction.resetToInitialState())

  }, [CommonState.getDietsNotToTakeSuccess, CommonState.getDietsNotToTakeFailure])
  ...
  useEffect(() => {
    if (state.getDiseaseDietNotToTakeSuccess) {
      setIsLoading(false)
      setAllMappedData(state.getDiseaseDietNotToTakeSuccess.data)
      addToast("Success!", {
                appearance: "success",
                content: `Get All data Success.`,
              });
    }
    if (state.getDiseaseDietNotToTakeFailure) {
      setIsLoading(false)
            addToast("Error!", {
                appearance: "error",
  ...
  }, [state.getDiseaseDietNotToTakeSuccess, state.getDiseaseDietNotToTakeFailure])


  useEffect(() => {

    if (state.createDiseaseDietNotToTakeSuccess) {
    ...
    if (state.createDiseaseDietNotToTakeFailure) {
      setIsSubmiting(false)
      addToast("Error!", {
        appearance: "error",
  ...
  }, [state.createDiseaseDietNotToTakeSuccess, state.createDiseaseDietNotToTakeFailure])
  ...
  let handleUpdate = (e) =>{
    ...
    if(allDietsChecked.length > 0  && requestPayload.id && requestPayload.disease_id){
      setIsSubmiting(true) 
      dispatch(ACTIONS.upateDiseaseDietNotToTake({
  ...
  let handleSubmit = (e) => {
    ...
    if(allDiseasesChecked.length > 0 && allDietsChecked.length > 0){
      setIsSubmiting(true) 
      dispatch(ACTIONS.createDiseaseDietNotToTake({
  ...
  let changeMode = (index,mode)=>{
    ...
    console.log(allMappedData[index] ,'data')
  ...
  useEffect(() => {
    if (state.updateDiseaseDietNotToTakeSuccess) {
      setRequestPayload({
      })
        addToast("Success!", {
            appearance: "success",
    ...
    if (state.updateDiseaseDietNotToTakeFailure) {
      setIsSubmiting(false)
        addToast("Error!", {
            appearance: "error",
...
}, [state.updateDiseaseDietNotToTakeSuccess, state.updateDiseaseDietNotToTakeFailure])
              ...
              {haveResponse &&
                <div className={customModalStyle.customModalLayout}>
                  <div className={customModalStyle.customModalBox}>
                    ...
                    <CommonText en="Diet Mapped Successfully." hi="आहार सफलतापूर्वक मैप किया गया।"/>
                    </p>
                  ...
                  <div className="row">
                    <div className="col-md-8">
                      <h1 className={FormStyle.formTitle}>
                        ...
                        <CommonText en="Disease Map Diet Not To Take" hi="रोग आधारित आहार जो नही लेना है" />
                    ...
                    <form onSubmit={(e)=>handleUpdate(e)}>
                      ...
                      <div className="col-md-12">
                          ...
                          <h1 className={`d-flex align-items-center ${FormStyle.formTitle}`}>
                          <GiMedicines className='me-2'/>
                          <CommonText en="Diets Not to take" hi="आहार जो नही लेना है" />
                      </h1>
                          </div>
                        </div>
...
export default Map_Disease_with_diets_not_to_take