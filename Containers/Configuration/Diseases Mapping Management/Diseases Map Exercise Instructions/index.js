...
function Map_Disease_with_exercise_instructions() {
  ...
  let state = useSelector((state) => state.DiseaseMapExerciseInstructionReducer);
  const CommonState = useSelector((state) => state.commonReducer);

  useEffect(() => {
    setIsLoading(true)
    dispatch(ACTIONS.getDiseaseExerciseInstruction())
    dispatch(CommonAction.getDataNotMappedWithDiseaseByParam('exercise_instruction'))
    
  }, [state.createDiseaseExerciseInstructionSuccess,state.updateDiseaseExerciseInstructionSuccess])
  ...
  useEffect(() => {
    if (CommonState.getExerciseInstructionSuccess) {
      setIsLoading(false)
      setAllInstructions(CommonState.getExerciseInstructionSuccess.allInstructions)
      addToast("Success!", {
                appearance: "success",
  ...
  useEffect(() => {
    if (state.getDiseaseExerciseInstructionSuccess) {
      setIsLoading(false)
      setAllMappedData(state.getDiseaseExerciseInstructionSuccess.data)
      addToast("Success!", {
                appearance: "success",
                content: `Get All data Success.`,
              });
    }
    if (state.getDiseaseExerciseInstructionFailure) {
      setIsLoading(false)
            addToast("Error!", {
                appearance: "error",
  ...
  }, [state.getDiseaseExerciseInstructionSuccess, state.getDiseaseExerciseInstructionFailure])
  ...
  let handleSubmit = (e) => {
    ...
    if(allDiseasesChecked.length > 0 && allInstructionsChecked.length > 0){
      setIsSubmiting(true)
      dispatch(ACTIONS.createDiseaseExerciseInstruction({
  ...
  useEffect(() => {
    ...
    if (state.createDiseaseExerciseInstructionSuccess) {
    ...
    if (state.createDiseaseExerciseInstructionFailure) {
      setIsSubmiting(false)
      addToast("Error!", {
        appearance: "error",
  ...
  }, [state.createDiseaseExerciseInstructionSuccess, state.createDiseaseExerciseInstructionFailure])
  ...
  let handleUpdate = (e) =>{
    ...
    if(allInstructionsChecked.length > 0  && requestPayload.id && requestPayload.disease_id){
      setIsSubmiting(true)
      dispatch(ACTIONS.updateDiseaseExerciseInstruction({
  ...
  useEffect(() => {
    if (state.updateDiseaseExerciseInstructionSuccess) {
      setRequestPayload({
      })
        addToast("Success!", {
            appearance: "success",
    ...
    if (state.updateDiseaseExerciseInstructionFailure) {
      setIsSubmiting(false)
        addToast("Error!", {
            appearance: "error",
...
}, [state.updateDiseaseExerciseInstructionSuccess, state.updateDiseaseExerciseInstructionFailure])
                  ...
                  <div className="row">
                    <div className="col-md-8">
                      <h1 className={FormStyle.formTitle}>
                        ...
                        <CommonText en="Disease Map Exercise Instruction" hi="रोग आधारित व्यायाम निर्देश" />
                    ...
                    <form onSubmit={(e)=>handleUpdate(e)}>
                      ...
                      <div className="col-md-12">
                          ...
                          <h1 className={`d-flex align-items-center ${FormStyle.formTitle}`}>
                          <GiMedicines className='me-2'/>
                          <CommonText en="Exercise Instructions" hi="व्यायाम निर्देश" />
...
export default Map_Disease_with_exercise_instructions