...
function ToungeObservation() {
  ...
  useEffect(() => {
    setIsLoading(true)
    dispatch(CommonAction.getToungeObservation())
  }, [state.createToungeObservationSuccess,state.updateToungeObservationSuccess])

  
  useEffect(() => {
    if (CommonState.getToungeObservationSuccess) {
      setIsLoading(false)
      setAllObservation(CommonState.getToungeObservationSuccess.observations)
      addToast("Success!", {
                appearance: "success",
                content: `Get All observation Success.`,
              });
    }
    if (CommonState.getToungeObservationFailure) {
      setIsLoading(false)
            addToast("Error!", {
                appearance: "error",
  ...
  }, [CommonState.getToungeObservationSuccess, CommonState.getToungeObservationFailure])


  useEffect(() => {

    if (state.createToungeObservationSuccess) {
    ...
    if (state.createToungeObservationFailure) {
      setIsSubmiting(false)
      addToast("Error!", {
        appearance: "error",
  ...
  }, [state.createToungeObservationSuccess, state.createToungeObservationFailure])
  ...
  let handleSubmit = (e) => {
    ...
    if(currentMode === 'Edit'){
      dispatch(ACTIONS.updateToungeObservation(observationPayload))
    }
    else{
      if(currentMode !== 'view'){
        dispatch(ACTIONS.createToungeObservation(observationPayload));
      }
    }
  }
  ...
  useEffect(() => {
    if (state.updateToungeObservationSuccess) {
      setObservationPayload({
        UID:"",
        observation : "",
      })
    ...
    if (state.updateToungeObservationFailure) {
      setIsSubmiting(false)
        addToast("Error!", {
            appearance: "error",
...
}, [state.updateToungeObservationSuccess, state.updateToungeObservationFailure])
...
export default ToungeObservation